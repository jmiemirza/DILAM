...
def eval_yolo_ckpts2(net, args, scenario, baseline_str, ckpts=None, ckpt_idx=0):
    """
        Used to evalute checkpoints from multi run with diff task order for some
        freezing & disjoint.
        Evaluate yolov3 chekpoints from previous runs.
    ...
    for idx, args.task in enumerate(tasks):
        ...
        if baseline_str == 'disjoint':
            # load ckpt
            if args.task == 'initial':
                net.load_state_dict(torch.load(args.ckpt_path, map_location=device))
            else:
                ckpt_folder = join(args.checkpoints_path, args.dataset, args.model, baseline_str, scenario)

                ckpt_offset = 4 if baseline_str == 'disjoint' else 5
                log.info(f'ckpt_offset {ckpt_offset}, idx {ckpt_idx}')
                ckpt_folder = join(ckpt_folder, ckpts[baseline_str][scenario][args.task] + f'{ckpt_offset + ckpt_idx}', 'weights')
                ckpt_path = join(ckpt_folder, 'best.pt')
                log.info(f'Loading: {ckpt_path}')
                ckpt = torch.load(ckpt_path, map_location=device)  # load checkpoint
                state_dict = ckpt['model'].float().state_dict()  # to FP32
        ...
        for i in range(0, idx + 1):
            ...
            if i == idx:
                ...
                severity_str = '' if args.task == 'initial' else f'{args.severity}'
                results.add_result(baseline_str, f'{tasks[idx]} {severity_str}', mean_result, scenario)
                results.add_result_map50to95(baseline_str, f'{tasks[idx]} {severity_str}', mean_result_map50to95, scenario)
